<template>
  <div class="flex flex-wrap">
    <div class="w-full px-4">
      ...
      >
        <div class="px-6 py-4">
          <h3 class="text-2xl mb-4">Add Pegawai</h3>
          <form @submit.prevent="createPegawai">
            <div class="mb-3 pt-0">
              ...
              <input
                ...
                required
              />
              <span v-if="errors.nip" class="text-red-500 text-xs">{{
                errors.nip
              }}</span>
            </div>
            <div class="mb-3 pt-0">
...
export default {
  data() {
    return {
      ...
      errors: {}, // untuk menyimpan pesan kesalahan
    };
  },
  methods: {
    createPegawai() {
      axios
        .post("http://localhost:8000/api/pegawai", this.form)
        .then(() => {
          Swal.fire({
            icon: "success",
            title: "Success",
        ...
        .catch((error) => {
          ...
          if (
            error.response &&
            error.response.data.message === "NIP already exists"
          ) {
            this.errors.nip = "NIP sudah ada. Gunakan NIP yang lain.";
            Swal.fire({
              icon: "error",
              title: "Error",
              text: "NIP sudah ada. Gunakan NIP yang lain.",
            });
          } else {
            Swal.fire({
              icon: "error",
